...
public class MazeGenerator
{
    ...
    readonly int minLeafSize;
    readonly int maxLeafSize;

    public MazeGenerator(RoomBuilder.GridSpaceType[,] grid)
    {
        gridArray = grid;
    }
    ...
    public class MazeClearing
    {
        ...
        public MazeClearing(int x, int y, int w, int h, int minClearing)
        {
            X = x;
            Y = y;
            width = w;
            height = h;
            minLeafSize = minClearing;
        }
        ...
        public bool Split()
        {
            ...
            if (splitHorizontal)
            {
                firstChild = new MazeClearing(X, Y, width, split, minLeafSize);
                secondChild = new MazeClearing(X, Y + split, width, height - split, minLeafSize);
            }
            else
            {
                firstChild = new MazeClearing(X, Y, split, height, minLeafSize);
                secondChild = new MazeClearing(X + split, Y, width - split, height, minLeafSize);
            }
            return true;
        }
        public void AssignRooms(RoomBuilder.GridSpaceType[,] gridArray)
        {
            if (!IsLeaf())
            {
                ...
                if (firstChild != null)
                {
                    firstChild.AssignRooms(gridArray);
                }
                if (secondChild != null)
                {
                    secondChild.AssignRooms(gridArray);
                }
        ...
        public void FillFloors(RoomBuilder.GridSpaceType[,] gridArray)
        {
            int roomX = (int)room.x;
            int roomY = (int)room.y;
            int roomWidth = (int)room.z;
            int roomHeight = (int)room.w;

            if (!IsLeaf())
            {
                // this isn't a leaf, so go into children to fill in floors
                if (firstChild != null)
                {
                    firstChild.FillFloors(gridArray);
                }
                if (secondChild != null)
                {
                    secondChild.FillFloors(gridArray);
                }
            }
            else
            {
    ...
    public void CreateClearings(int width, int height, int minLeafSize, int maxLeafSize, bool fillRooms)
    {
        List<MazeClearing> clearingList = new();
        MazeClearing root = new(1, 1, width, height, minLeafSize);
        ...
        root.AssignRooms(gridArray);
        if (fillRooms)
        {
            root.FillFloors(gridArray);
        }
    }
}
